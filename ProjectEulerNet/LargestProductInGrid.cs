...
namespace ProjectEulerNet
{
    ...
    {
        private static int[][] _grid = new int[20][]
        {
        ...
        public static long Solve(int n)
        {
            ...
            for (var i = 0; i < _grid.GetLength(0); i++)
            {
                ...
                {
                    res = Math.Max(res, FindMaxForCurrentPosition(n, i, j));
                }
            }
        ...
        private static long FindMaxForCurrentPosition(int count, int x, int y)
        {
            long n = 1, ne = 1, e = 1, se = 1, s = 1, sw = 1, w = 1, nw = 1;
            var gridLength = _grid.GetLength(0);

            for (var i = 0; i < count; i++)
            {
                if (x - i >= 0)
                    n *= _grid[x - i][y];
                if (x - i >= 0 && y + i < gridLength)
                    ne *= _grid[x - i][y + i];
                if (y + i < gridLength)
                    e *= _grid[x][y + i];
                if (x + i < gridLength && y + i < gridLength)
                    se *= _grid[x + i][y + i];
                if (x + i < gridLength)
                    s *= _grid[x + i][y];
                if (x + i < gridLength && y - i >= 0)
                    sw *= _grid[x + i][y - i];
                if (y - i >= 0)
                    w *= _grid[x][y - i];
                if (x - i >= 0 && y - i >= 0)
